...
## Recent Changes
- 2025-10-28: تحسينات رئيسية في استخراج البيانات من الجداول
  - **إصلاح استخراج المركز الانتخابي**: تم تحديث الكود للعمل مع بنية الجدول HTML الفعلية
  - استخراج البيانات من صفوف الجدول (<tr>/<td>) مباشرة
  - التعامل الصحيح مع الجداول العربية حيث:
    * العمود الأيمن (cells[1]) = التسمية (مثل "مركزك الانتخابي")
    * العمود الأيسر (cells[0]) = القيمة (مثل "الوحدة المحلية براس الحكمة")
  - ثلاث طرق متعددة للاستخراج:
    * الطريقة 1: استخراج مباشر من صفوف الجدول باستخدام Selenium
    * الطريقة 2: استخراج من الجدول باستخدام BeautifulSoup
    * الطريقة 3: البحث في العناصر النصية كنسخة احتياطية
  - رسائل تأكيد مفصلة عند نجاح استخراج كل حقل
  - زيادة وقت الانتظار إلى 5 ثوان لضمان تحميل البيانات
